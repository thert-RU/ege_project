...
class Base {
    ...
    await fetch(url)
     ...
     .then(data_json => {
        ...
        let data = data_json
        let list = this.getTasksAndThemesList(data['tasks'])
...
class TaskWidget{
    constructor(widget){
        ...
        this.form = new Form('/change', 'post', 'submit')
        this.files = []
        this.nodes = []
        this.count_nodes = 0
    ...
    async createFictiveBase(url) {
        let fictive_base = new Base()
        await fictive_base.main(url, false)
    }
    show() {
...
async function base(){
    ...
    let base_red_btn = document.querySelector('.button-to-base_red')
    base_red_btn.addEventListener('click', ()=>{
        let form = new Form('/base_red', 'get', 'info')
        start_widget.appendChild(form.form)
        form.input.click()
    })
    ...
    let select_text = document.querySelector('.type-select p')
    ...
    let base = new Base(base_widget, select, select_text, inner, red=true)
...
async function change(){
    ...
    let add_task = new TaskWidget(task_widget)
    await add_task.createFictiveBase('/ege project/data.json')
